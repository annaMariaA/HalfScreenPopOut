...
rtdat$targSide[rtdat$var=="homo" & rtdat$condition=="full" & rtdat$targSide!="absent"] = "homogeneous"
rtdat$targSide[rtdat$var=="hetero" & rtdat$condition=="full" & rtdat$targSide!="absent"] = "heterogeneous"

accdat =aggregate(data=rtdat, acc ~ subj + targSide + version + condition, FUN="mean")
errorbar <- summarySEwithin(accdat, measurevar="acc", withinvars=c("targSide","version", "condition"), idvar="subj")
pAcc2 = ggplot(errorbar, aes(x=targSide, y=100*acc, fill=condition)) + geom_bar(stat="identity", position="dodge") + theme_minimal()
pAcc2 = pAcc2 + scale_y_continuous(name="Accuracy (%)") + scale_x_discrete(name="TargetSide")+scale_fill_manual(name="Split/full screen", values=cbPalette) + facet_wrap(~version)
pAcc2 = pAcc2 + geom_errorbar(position=position_dodge(.9), within=.25, aes(ymin=(acc-ci)*100,ymax=(acc+ci)*100),width=.5)
ggsave("accuracy.jpg",dpi=600, width=6, height=3)

RTdat =aggregate(data=rtdat, RT ~ subj + targSide + version + condition, FUN="mean")
errorbar <- summarySEwithin(RTdat, measurevar="RT", withinvars=c("targSide","version", "condition"), idvar="subj")
pAcc2 = ggplot(errorbar, aes(x=targSide, y=RT, fill=condition)) + geom_bar(stat="identity", position="dodge") + theme_minimal()
pAcc2 = pAcc2 + scale_y_continuous(name="RT (%)") + scale_x_discrete(name="TargetSide")+scale_fill_manual(name="Split/full screen", values=cbPalette) + facet_wrap(~version)
pAcc2 = pAcc2 + geom_errorbar(position=position_dodge(.9), within=.25, aes(ymin=(RT-ci),ymax=(RT+ci)),width=.5)
ggsave("RT.jpg",dpi=600, width=6, height=3)
