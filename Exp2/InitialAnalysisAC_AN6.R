...
dat <- read.csv("RtAcc.txt", sep="\t")
names(dat) = c("subj", "trialNum","easySide", "targPresent", "targSide", "RT", "acc", "var", "con")
dat = select(dat, subj, trialNum, easySide, targPresent, targSide, RT, acc, var, con)
...
rtdat = data.frame(subj=dat$subj, trial=dat$trial, targSide=dat$targSideRel, RT=dat$RT, acc=dat$acc, easySide=dat$easySide,var=dat$var, condition=dat$con )
# we don't want to be looking at RTs for incorrect trials
rtdat$RT[rtdat$acc==0] = NaN

# save!!!
saveRDS(rtdat,file="processedRTandAccData.Rda")
...
dat <- read.csv("Fix.txt", header=T, sep="\t",
	colClass = c(
		"subj"="factor", 
		"trialNum"="numeric", 
		"fixNum"="numeric",    
		...
		"var" = "factor",
		"con"="factor"))
names(dat) = c("subj", "trialNum","fixNum", "fixX","fixY","fixOn","fixOff", "targPresent", "targSide", "easySide","var","condition")
...
# #we want to filter out all incorrect trials!
 ...
 accdat = readRDS(file="processedRTandAccData.Rda")
 dat$acc = 0
...
write.table(fixdat, "processedFixData.txt", sep=",")
